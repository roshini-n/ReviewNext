...
export class BeautyProductReviewService {
  ...
  private reviewsCollection = collection(this.firestore, 'beautyProductReviews');
  ...
  updateReview(reviewId: string, updates: Partial<Review>): Observable<Review> {
    const docRef = doc(this.firestore, `beautyProductReviews/${reviewId}`);
    return from(
  ...
  deleteReview(id: string): Observable<void> {
    const docRef = doc(this.firestore, `beautyProductReviews/${id}`);
